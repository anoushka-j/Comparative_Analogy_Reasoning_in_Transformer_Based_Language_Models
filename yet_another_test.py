    ...
    for model_name, (tokenizer, model, is_t5, is_gpt2) in models_and_tokenizers.items():
        ...
        if model_name in ["BERT", "DistilBERT", "RoBERTa"]:
            ...
            for seq in t5_out:
                ...
                # Compute sequence probability as confidence

        # Compute per-token normalized confidence
                with torch.no_grad():
                    labels = seq.unsqueeze(0)  # shape: (1, seq_len)
                    outputs = model(**inputs, labels=labels)
                    logits = outputs.logits  # (1, seq_len, vocab_size)
                    log_probs = F.log_softmax(logits, dim=-1)

                    # token-wise log-probabilities for the generated sequence
                    token_log_probs = log_probs[0, torch.arange(labels.size(1)), labels[0]]

                    # Compute geometric mean of token probabilities to normalize across sequence length
                    seq_prob_normalized = torch.exp(token_log_probs.mean()).item()  # between 0 and 1


                preds.append(pred)
                confs.append(seq_prob_normalized)
        ...
        with torch.no_grad():
            ...
            if model_name in ["BERT", "DistilBERT", "RoBERTa"]:
                # Use hidden states (MaskedLMOutput does NOT expose last_hidden_state)
                ...
                # Encoder hidden state contains semantic representation
                outputs = model.encoder(**inputs, output_hidden_states=True)
                last_hidden = outputs.hidden_states[-1]
                emb = last_hidden.mean(dim=1)

            elif model_name == "GPT-2":
                # GPT-2 has no mask → use final token embedding
                outputs = model(**inputs, output_hidden_states=True)
                last_hidden = outputs.hidden_states[-1]
                emb = last_hidden[:, -1, :]  # (batch, hidden_dim)

            else:
                raise ValueError(f"Unknown model {model_name}")

        mean_emb = emb.squeeze().detach().numpy()
