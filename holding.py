...
#importing the analogy dataset
from dataset import benchmark 

# ============================================================
# Helper functions
# ============================================================
correct_answer = ["ship", "boat", "vessel"]
...
def semantic_similarity(model, tokenizer, word, correct_answers):
    ...
    return max(sims)  # take the best match among correct answers
...
def diversity(preds):
    ...
    valid_analogies = 0 
    for p in preds : 
        if p.lower().strip() in correct_answer : 
            valid_analogies += 1
    print(valid_analogies)
    return valid_analogies / len(preds)
...
t5_text = "fill mask: A pilot works for an airline, just like a sailor works for a <extra_id_0>."
...
for seq in t5_out:
    decoded = t5_tokenizer.decode(seq)

    pred = decoded.replace("<pad>", "") \
                  .replace("</s>", "") \
                  .replace("<extra_id_0>", "") \
                  .replace("<extra_id_1>", "") \
                  .strip()
    ...
    if pred not in t5_preds:
        t5_preds.append(pred)
        t5_confs.append(conf)

# Use top-1 confidence
t5_conf_scalar = t5_confs[0] if t5_confs else 0.0

# Top-1 similarity for T5
t5_sim_scalar = sim(t5_preds[0]) if t5_preds else 0.0
...
def top1_accuracy(preds):
    return 1.0 if preds and preds[0].lower().strip() in correct_answer else 0.0


def topk_accuracy(preds):
    return 1.0 if any(p.lower().strip() in correct_answer for p in preds) else 0.0


eval_results = {
    "Model":             ["BERT", "DistilBERT", "RoBERTa", "T5", "GPT-2"],
    "Top-1 Accuracy":    [
        top1_accuracy(bert_preds),
        top1_accuracy(distil_preds),
        top1_accuracy(roberta_preds),
        top1_accuracy(t5_preds),
        top1_accuracy(gpt2_preds)
    ],
    "Top-10 Accuracy":   [
        topk_accuracy(bert_preds),
        topk_accuracy(distil_preds),
        topk_accuracy(roberta_preds),
        topk_accuracy(t5_preds),
    ...
    "Diversity": [
        diversity(bert_preds),
        diversity(distil_preds),
        diversity(roberta_preds),
        diversity(t5_preds),
